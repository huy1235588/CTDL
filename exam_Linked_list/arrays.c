...
int binary_search(int __array[], int __size, int __key) {
    ...
    while (low <= high) {
        ...
        if (__array[middle] == __key) { // Tìm được phần tử cần tìm
            return middle;
        } else {
            if (__array[middle] > __key) {  // Phần tử cần tìm thuộc nửa dưới
                high = middle - 1;
            } else {    // Phần tử cần tìm thuộc nửa trên
                low = middle + 1;
            }
        }
    }
...
void selection_sort(int __array[], int __size) {
    ...
    for (int i = 0; i < __size - 1; i ++) {
        ...
        for (int j = i + 1; j < __size; j ++) {
            if (__array[min_index] > __array[j]) {  // nghịch thế
                min_index = j;
            }
        }
...
void interchange_sort(int __array[], int __size) {
    ...
    for (int i = 0; i < __size - 1; i ++) {
        for (int j = i + 1; j < __size; j ++) {
            if (__array[i] > __array[j]) {  // nghịch thế
...
void bubble_sort(int __array[], int __size) {
    ...
    for (int i = 0; i < __size - 1; i ++) {
        ...
        for (int j = 0; j < __size - 1 - i; j ++) {
            if (__array[j] > __array[j + 1]) {  // nghịch thế
...
void insertion_sort(int __array[], int __size) {
    ...
    for (int index = 1; index < __size; index ++) {
        ...
        while (pos > 0 && __array[pos - 1] > x) {   // nghịch thế
...
void shaker_sort(int __array[], int __size) {
    ...
    while(left < right) {
        ...
        for (index = left; index < right; index ++) {
            if (__array[index] > __array[index + 1]) {  // nghịch thế
        ...
        for (index = right; index > left; index --) {
            if (__array[index - 1] > __array[index]) {  // nghịch thế
...
void shell_sort(int __array[], int __size) {
    ...
    for (h = __size/2; h > 0; h /= 2) {
        for (index = h; index < __size; index ++) {
            pos = index;
            x = __array[index];
            while (pos >= h && __array[pos - h] > x) {    // nghịch thế
...
static void __partition(int a[], int low, int high){
    ...
    j = high; // số chỉ phải
    do {
        while (a[i] < x && i < high) {  // tìm phần tử bên trái lớn hơn trục
            i++;
        }
        while (a[j] > x && j > low) {   // tìm phần tử bên phải nhỏ hơn trục
            j--;
        }
        if (i <= j) {
            swapInteger(&a[i], &a[j]);
...
void enter_dynamic_array_of_data(__DATA* __array, int __size, int __quantity) {
    for (int index = __size - __quantity; index < __size; index++) {
        ...
        scanf("%d", &(__array + index)->number); getchar();
...
static void swap__DATA(__DATA* this, __DATA* that) {
    __DATA temp = *this;
    *this = *that;
    *that = temp;
}

void selection_sort_in_array_of_data(__DATA* __array, int __size) { // sắp xếp theo __text tăng dần
    if (__size <= 1) return;
    int min_index;
    for (int i = 0; i < __size - 1; i ++) {
        min_index = i;  // số chỉ của phần tử có giá trị nhỏ nhất trong mỗi lần duyệt
        for (int j = i + 1; j < __size; j ++) {
            if ( strcmp((__array + min_index)->text, (__array + j)->text) > 0) {  // nghịch thế
                min_index = j;
            }
        }
        // Kết thúc mỗi lần duyệt, kiểm tra xem có tìm được phần tử có giá trị nhỏ nhất
